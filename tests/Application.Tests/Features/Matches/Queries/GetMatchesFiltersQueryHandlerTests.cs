...
namespace Application.Tests.Features.Matches.Queries
{
    public sealed class GetMatchesFiltersQueryHandlerTests
    {
        ...
        public async Task Handle_Should_Return_Matches_For_Tournament()
        {
            ...
            {
                new() { Id = Guid.NewGuid(), TournamentId = tournamentId, MatchDate = DateTime.UtcNow, Stage = MatchStage.Group, Status = MatchStatus.Scheduled },
                new() { Id = Guid.NewGuid(), TournamentId = tournamentId, MatchDate = DateTime.UtcNow.AddHours(2), Stage = MatchStage.Group, Status = MatchStatus.Scheduled },
                new() { Id = Guid.NewGuid(), TournamentId = Guid.NewGuid(), MatchDate = DateTime.UtcNow.AddHours(1), Stage = MatchStage.Group, Status = MatchStatus.Scheduled }
            };
            ...
            var handler = new GetMatchesByTournamentQueryHandler(dbMock.Object, mapper);
        ...
        public async Task Handle_Should_Return_Matches_For_Date()
        {
            ...
            var matches = new List<Domain.Entities.Match>
            {
                new() { Id = Guid.NewGuid(), MatchDate = start.AddHours(10), Stage = MatchStage.Group, Status = MatchStatus.Scheduled },
                new() { Id = Guid.NewGuid(), MatchDate = start.AddHours(22), Stage = MatchStage.Group, Status = MatchStatus.Scheduled },
                new() { Id = Guid.NewGuid(), MatchDate = end.AddMinutes(1), Stage = MatchStage.Group, Status = MatchStatus.Scheduled }
            };
            ...
            var handler = new GetMatchesByDateQueryHandler(dbMock.Object, mapper);
